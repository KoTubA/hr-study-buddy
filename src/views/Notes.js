...
const Notes = () => {
  ...
  return (
    <Wrapper>
      ...
      {isLoading ? <h2>Loading...</h2> : <NotesWrapper>{data.notes.length ? data.notes.map(({ title, content, id }) => <Note id={id} key={id} title={title} content={content} />) : <p>Create your first note</p>}</NotesWrapper>}
    </Wrapper>
  );
};
